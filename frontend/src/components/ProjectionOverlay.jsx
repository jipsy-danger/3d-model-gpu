...
void main(){vec2 p=gl_PointCoord-.5;if(dot(p,p)>.25)discard;gl_FragColor=vec4(uColor,uOpacity);}`;
...
function baseUniforms(camera){return {
  uPosition:{value:new THREE.Vector3().fromArray(camera.position||[0,0,0])},
  uRight:{value:new THREE.Vector3().fromArray(camera.right||[1,0,0])},
  uUp:{value:new THREE.Vector3().fromArray(camera.up||[0,1,0])},
  uForward:{value:new THREE.Vector3().fromArray(camera.forward||[0,0,1])},
  uTanHalf:{value:Math.tan((Number(camera.fov)||48)*Math.PI/360)},
  uAspect:{value:Math.max(.1,Number(camera.aspect)||1)}, uFitScale:{value:1}, uFitCenter:{value:new THREE.Vector2()},
  uColor:{value:new THREE.Color('#9bf5ff')}, uOpacity:{value:1}
};}

function syncUniforms(material,camera,center,scale){
  material.uniforms.uPosition.value.fromArray(camera.position); material.uniforms.uRight.value.fromArray(camera.right);
  material.uniforms.uUp.value.fromArray(camera.up); material.uniforms.uForward.value.fromArray(camera.forward);
  material.uniforms.uTanHalf.value=Math.tan((Number(camera.fov)||48)*Math.PI/360);
  material.uniforms.uAspect.value=Math.max(.1,Number(camera.aspect)||1); material.uniforms.uFitCenter.value.copy(center); material.uniforms.uFitScale.value=scale;
}
...
function createSurface(vertices,camera){
  ...
  const positions=[]; for(const i of indices)positions.push(...vertices[i].map(Number));
...
function createLine(points,camera,color='#76ff91',width=3){
  ...
  const uniforms=baseUniforms(camera); uniforms.uColor.value.set(color);
  ...
  const object=new THREE.Line(geometry,material); object.renderOrder=1000; object.frustumCulled=false; return {object,geometry,material,width};
}
...
export default function ProjectionOverlay(){
  useEffect(()=>{
    ...
    const render=()=>{
      const {one,two}=roots(); if(!one||!two||!frame?.input?.vertices||!camera)return;
      if(!syncEnabled()){disposeScene('one');disposeScene('two');return;}
      const vertices=frame.input.vertices.map(v=>v.map(Number)),r1=ensureRenderer('one',one),r2=ensureRenderer('two',two); if(!r1||!r2)return;
      const s1=scenes.get('one'),s2=scenes.get('two'); s1.clear();s2.clear();
      ...
      const fit1={center:new THREE.Vector2(),scale:1};
      const p1=createPoints(vertices,camera,axis,5.5);syncUniforms(p1.material,camera,fit1.center,fit1.scale);s1.add(p1.object);
      const cWorld=Array.isArray(frame.centroid)?frame.centroid.map(Number):[0,0,0];
      const centroidScreen=camera.centroid_screen;
      if(centroidScreen&&Number.isFinite(Number(centroidScreen.x))){
        const guide=createLine([[-100,cWorld[1],cWorld[2]],[100,cWorld[1],cWorld[2]]],camera,'#76ff91',2);syncUniforms(guide.material,camera,fit1.center,fit1.scale);s1.add(guide.object);
      ...
      if(size?.length){const hx=Number(size[0])/2;const xline=createLine([[cWorld[0]-hx,cWorld[1],cWorld[2]],[cWorld[0]+hx,cWorld[1],cWorld[2]]],camera,'#76ff91',3);syncUniforms(xline.material,camera,fit.center,fit.scale);s2.add(xline.object);}
    ...
    const poll=()=>{if(dead)return;if(!wsConnected)loadState();render();pollTimer=setTimeout(poll,100);};
    const framePoll=()=>{if(dead)return;loadFrame();frameTimer=setTimeout(framePoll,250);};
